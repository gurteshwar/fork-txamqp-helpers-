...
class AmqpProtocol(AMQClient):
    ...
    def read(self, exchange, queue, routing_key, callback):
        """Add an exchange to the list of exchanges to read from."""
        if self.connected:
            # Connection is already up. Add the reader.
            self.setup_read(exchange, queue, routing_key, callback)
        else:
    ...
    @inlineCallbacks
    def setup_read(self, exchange, queue, routing_key, callback):
...
class AmqpFactory(protocol.ReconnectingClientFactory):
    ...
    def read(self, exchange=None, queue=None,routing_key=None, callback=None):
        """Configure an exchange to be read from."""
        assert(exchange != None and routing_key != None and callback != None and queue != None)

        # Add this to the read list so that we have it to re-add if we lose the connection.
        self.read_list.append((exchange, queue, routing_key, callback))

        # Tell the protocol to read this if it is already connected.
        if self.p != None:
            self.p.read(exchange, queue, routing_key, callback)
